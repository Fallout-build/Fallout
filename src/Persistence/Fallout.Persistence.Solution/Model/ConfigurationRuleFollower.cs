...
internal readonly ref struct ConfigurationRuleFollower(IReadOnlyList<ConfigurationRule>? configurationRules)
{
    ...
    internal bool HasRules => !configurationRules.IsNullOrEmpty();

    internal readonly bool? GetIsBuildable(string? solutionBuildType = null, string? solutionPlatform = null)
    {
        string value = GetDimensionValue(BuildDimension.Build, solutionBuildType, solutionPlatform);
    ...
    {
        string value = GetDimensionValue(BuildDimension.Deploy, solutionBuildType, solutionPlatform);
        return string.IsNullOrEmpty(value) ? null : bool.Parse(value);
    }

    internal readonly string? GetProjectBuildType(string solutionBuildType, string solutionPlatform)
    {
        string value = GetDimensionValue(BuildDimension.BuildType, solutionBuildType, solutionPlatform);
    ...
    {
        string value = GetDimensionValue(BuildDimension.Platform, solutionBuildType, solutionPlatform);
    ...
    /// </summary>
    private static bool RuleAppliesTo(ConfigurationRule rule, BuildDimension dimension, string? solutionBuildType,
        string? solutionPlatform)
    {
        ...
        static bool IsSamePlatform(ConfigurationRule rule, string solutionPlatform) =>
            StringComparer.OrdinalIgnoreCase.Equals(PlatformNames.Canonical(rule.SolutionPlatform),
                PlatformNames.Canonical(solutionPlatform));
    }

    private readonly string GetDimensionValue(BuildDimension dimension, string? solutionBuildType, string? solutionPlatform)
    {
        foreach (ConfigurationRule rule in configurationRules.GetStructReverseEnumerable())
        {
