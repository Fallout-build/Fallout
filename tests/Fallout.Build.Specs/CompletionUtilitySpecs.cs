...
public class CompletionUtilitySpecs
{
    ...
    public async Task TestGetCompletionItemsTargetBuild()
    {
        ...
        var items = CompletionUtility.GetItemsFromSchema(schema, new[]
        {
            "dev"
        });
    ...
    [InlineData("-ApiKey ", new[]
    {
        "--target",
        "--nuget-source"
    })]
    [InlineData("--api-key ", new[]
    {
        "--target",
        "--nuget-source"
    })]
    [InlineData("--target ", new[]
    {
        "Compile",
        "GitHub-Publish",
        "--api-key",
